...
const emptyForm: PositionFormState = {
  ...
  requirements: '', skillTags: [], expiresAt: '', headcountCode: '', requestedHeadcount: '1',
  costCenterCode: '', costCenterName: '', budgetAmount: '0', budgetCurrency: 'CNY', businessJustification: '',
}
...
export function CompanyPositionForm() {
    ...
    try {
      const saved = await request<RecruitmentJob>(editing ? `/v1/company/recruitment/positions/${id}` : '/v1/company/recruitment/positions', {
        ...
        body: JSON.stringify({
          ...
          requisition: {
            headcountCode: form.headcountCode.trim(), requestedHeadcount: Number(form.requestedHeadcount),
            costCenterCode: form.costCenterCode.trim(), costCenterName: form.costCenterName.trim() || undefined,
            budgetAmount: Number(form.budgetAmount), budgetCurrency: form.budgetCurrency.trim().toUpperCase(),
  ...
  return <div className="space-y-6">
    ...
    <form className="space-y-5" onSubmit={submit}>
      ...
      <Card><SectionTitle title="招聘授权" description="明确该岗位占用的编制、成本中心和可审核预算；这些信息仅对企业 HR 与超级管理员可见。" /><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="编制编码" required><input required maxLength={64} className={inputClass} value={form.headcountCode} onChange={event => update('headcountCode', event.target.value)} placeholder="例如 HC-RD-2026-014" /></Field><Field label="申请人数" required><input required min="1" max="1000" type="number" className={inputClass} value={form.requestedHeadcount} onChange={event => update('requestedHeadcount', event.target.value)} /></Field><Field label="成本中心编码" required><input required maxLength={64} className={inputClass} value={form.costCenterCode} onChange={event => update('costCenterCode', event.target.value)} placeholder="例如 CC-RD-01" /></Field><Field label="成本中心名称"><input maxLength={128} className={inputClass} value={form.costCenterName} onChange={event => update('costCenterName', event.target.value)} placeholder="例如 产品研发成本中心" /></Field><Field label="招聘预算" required><input required min="0" step="0.01" type="number" className={inputClass} value={form.budgetAmount} onChange={event => update('budgetAmount', event.target.value)} /></Field><Field label="预算币种" required><input required minLength={3} maxLength={3} className={inputClass} value={form.budgetCurrency} onChange={event => update('budgetCurrency', event.target.value)} placeholder="CNY" /></Field></div><div className="mt-4"><Field label="招聘理由" required><textarea required maxLength={2000} className={textareaClass} value={form.businessJustification} onChange={event => update('businessJustification', event.target.value)} placeholder="说明业务需求、团队缺口和预期到岗时间" /></Field></div></Card>
