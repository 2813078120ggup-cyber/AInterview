import { AlertTriangle, Building2, ChevronLeft, ChevronRight, Loader2, Plus, Search, UserRound, X } from 'lucide-react'
...
import { evaluateRoleSelection } from '@/lib/role-assignment'
...
export function AdminUsers() {
  ...
  const formRoleSelection = useMemo(
    () => evaluateRoleSelection(formRoles, form.roleIds, form.companyId || null),
    [form.companyId, form.roleIds, formRoles],
  )
  ...
  async function createUser() {
    ...
    if (!formRoleSelection.valid) {
      setError(formRoleSelection.message)
      return
    }
    setSaving(true)
    try {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4 sm:p-5"><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_180px_140px]">
      ...
      <ResponsiveSelect ariaLabel="状态" value={status} onValueChange={value => updateQuery({ status: value })} options={[{ value: '', label: '全部状态' }, { value: '1', label: '已启用' }, { value: '0', label: '已停用' }]} />
