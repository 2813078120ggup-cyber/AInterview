...
- 新增：注册页增加“企业 HR 注册”类型。企业注册要求联系人手机号验证，并收集企业全称、行业、规模、城市及可选官网、法定代表人、统一社会信用代码等资料；服务端事务内创建独立企业租户、首个 `COMPANY_ADMIN` 账号和角色绑定，成功后展示企业识别码并返回登录，不静默创建会话。
- 数据与安全：Flyway V47 为企业增加法定代表人和统一社会信用代码字段，后者建立唯一约束；企业注册接口由服务端生成 `companyCode`，不接受前端提交的 `companyId` 或角色，账号、联系方式、网站格式、验证码、租户归属均在服务端校验并写入认证审计。企业注册独立纳入公开接口限流，默认每 IP 每分钟 3 次。
- 变更：登录、个人注册、找回密码统一使用认证字体、暖色 Token 和正式化提示语；“忘记密码”和“立即注册”使用同一强调色。所有图形验证码统一为四位、自动转大写并限制输入长度，个人注册邮箱继续选填，企业资料字段按页面必填状态禁用主操作按钮。
- 验证：后端全量 349 项测试通过并将本机开发库从 V46 迁移至 V47；前端 `npx.cmd tsc --noEmit`、`npm.cmd run lint -- --no-fix`、`npm.cmd run build` 通过；认证 Playwright 6/6 通过，覆盖个人注册邮箱选填、企业 HR 注册资料、四页验证码和减少动态效果。Vite 构建保留既有 CodeEditor 大分块提示。本轮未重建 Docker，未提交、未推送。

### 认证验证码单行化与平台鼠标跟随开关

- 视觉：登录、个人注册和找回密码共用的图形验证码移除嵌套“卡片中卡片”结构，统一为与账号、密码输入框同高、同宽、同圆角的 48px 单行控件；验证码输入、图像和刷新按钮在同一行内完成，加载、失败、禁用、刷新、错误提示和挑战回调逻辑保持不变，390px 移动端无横向溢出。
- 新增：运维菜单增加独立 `/admin/theme-settings`“主题设置”，保留原有 AI 服务“平台设置”入口；超级管理员可通过可访问的开关统一启停候选人、企业和管理员工作区的鼠标跟随动画，保存成功后当前页面立即挂载或卸载跟随层。公开首页与认证页继续不显示跟随效果，系统“减少动态效果”偏好仍具有更高优先级。
- 数据与安全：Flyway V46 新增单例 `platform_ui_setting`，鼠标跟随默认开启并记录最后修改人和时间；公开 `GET /v1/platform/ui-settings` 只暴露无敏感信息的布尔状态，`PUT /v1/admin/platform/ui-settings` 继续要求认证及 `ADMIN` 方法权限，从服务端当前身份写入修改人并登记平台操作审计。前端首次读取失败时保留既有开启行为，但在读取完成前不短暂挂载动画。
- 验证：新增后端设置服务/控制器 5 项测试和管理员主题设置 Playwright 回归；认证 Playwright 同时校验三页验证码与相邻输入框等宽。后端完整测试、`test-compile`、40 条 Flyway 迁移校验、`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build` 和 `git diff --check` 通过；完整测试已将本机开发库从 V45 成功迁移至 V46。应用内浏览器实测桌面登录页账号、密码、验证码均为 355×48px，主题开关从开启切到关闭后跟随 DOM 由 2 个立即降为 0。本轮尚未重建 Docker，未提交、未推送。

### 认证与产品初始页 Docker 运行态同步

- 运维：仅重建并以 `--no-deps` 替换 `backend` 与 `frontend`，未执行 `down`、未删除卷，也未重启 MySQL、Redis、Gateway、注册中心或算法判题服务。后端新容器 `8a2b5a632b70` 使用镜像 `sha256:aaa403ef92e2335dc7498f3af0f4b6a8e64a20cfb5e27edd138be57ca623b16d` 并保持健康；前端新容器 `59eb696860fe` 使用镜像 `sha256:848dfbca8aa956944af9c559e8cd44bb0f4c20b3120af445c25ac3912193cfc9` 并正常运行。
- 验证：新镜像与运行容器的 Nginx 配置检查通过，容器内生产资源同时包含“招聘结论，都有来处”“每一面，都算数”和认证几何背景图；`/`、`/features`、`/login`、`/register`、`/forgot-password` 均返回 HTTP 200。图形验证码挑战接口返回 HTTP 200 和 `imageDataUrl`，不再返回旧滑块字段 `targetPercent`；未认证 `/api/v1/auth/me` 返回 401。
- 数据与边界：Flyway 启动日志验证 39 条迁移，数据库当前版本 V45 且无需新增迁移，后端启动日志未发现 `ERROR`、启动失败或异常。MySQL `cefabb94d4e8`、Redis `687234d18171`、Gateway `16527627e534`、注册中心 `5b87a7ddf578` 和判题服务 `5d3c17cf04cf` 的容器 ID 均保持不变。本轮未提交、未推送。

### 产品初始页品牌叙事与招聘证据链优化

- 视觉：保留 `/` 与 `/features` 既有暖色品牌、产品模拟界面、导航锚点和长页信息架构，精炼首屏说明并移除重复的决策提示；新增独立“招聘决策的证据链”区块，以一条连续链路呈现“系统记录、模型辅助、企业决定”，不使用通用三卡片、AI 科技渐变或持续装饰动画。
- 体验：桌面端证据链使用细分隔的横向阅读顺序，移动端折叠为带竖向连接的单列结构；首屏 CTA、真实产品预览和后续招聘流程保持原位置与行为。首页根节点补充 `#top` 锚点，页脚“回到顶部”现在指向真实目标。
- 验证：应用内浏览器确认 1280×900 与 390×844 下首屏层级、证据链、滚动固定顶栏和产品预览正常，移动端无横向溢出；新增首页证据链、平台/岗位入口、固定顶栏和移动端宽度 Playwright 专项回归并以 2/2 通过，`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build` 和 `git diff --check` 通过，生产构建仅保留既有 CodeEditor 大分块提示。本轮未重建 Docker，未提交、未推送。

### 认证三页实体卡片、统一顶栏与必填操作门禁

- 视觉：登录、个人注册和找回密码统一使用同一套奶油白实体卡片、暖灰细边框、28px 圆角和两层暖色浅阴影；注册与找回密码的步骤表单不再使用透明面板，页面标题、步骤进度和既有几何品牌背景保持不变。
- 导航：新增三页共用并固定于视口顶部的认证顶栏，复用产品首页的机器人品牌标记、`AInterview / 招聘协同与智能面试` 两级文字、内容宽度和底部分隔线；品牌及“返回首页”均指向产品初始页。桌面和移动端由认证壳层分别预留 76px 与 66px 顶部内边距，避免首个内容块的外边距塌陷后把顶栏和整个页面共同推离视口顶部；认证路由不再套用带 `transform` 的通用页面过渡容器，因此滚动时顶栏持续位于视口顶端且不遮挡标题或卡片。登录卡新增紧凑品牌识别区，以 `AInterview` 为标题、以“每一面，都算数。”为品牌主张，登录方式页签仍保持在卡片最顶部。
- 交互：密码登录、验证码登录、验证码发送、注册第一步、创建账户、账户验证和保存新密码的主操作按钮，全部由当前步骤的必填字段直接派生禁用状态；手机号、验证码目标或图形验证码未填写时不能提前发送或继续。注册手机号发生变化时同步作废旧的验证码填写状态，邮箱继续保持选填；接口、请求体、认证流程和路由未改变。
- 验证：应用内浏览器确认三页在 1280×900 和 390×844 下均为实体表面、共享同款顶栏且无横向溢出；修复后登录页顶栏 `top=0`、主体从 76px 开始，注册页滚动 86px 后顶栏仍保持 `top=0`，截图中的顶部空带不再出现。登录卡桌面中心点与视口中心一致，移动端顶栏为 66px 并保留“首页”返回入口。认证、注册、找回密码、固定顶栏、必填禁用、减少动态效果与账户密码安全 Playwright 回归 7/7 通过；`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build`、Compose 配置解析和 `git diff --check` 通过，生产构建仅保留既有 CodeEditor 大分块提示。本轮未重建 Docker，未提交、未推送。

### 认证系列几何品牌背景与克制动态

- 视觉：将项目几何背景图作为登录、个人注册和找回密码页面的统一固定背景，并新增共享 `AuthBrandBackground` 装饰组件；保留现有认证页面结构、字段、步骤、状态、路由、请求体和 API，不新增前端依赖。整体继续使用米白、奶油白、暖灰、灰褐、浅棕与深咖色，未加入蓝紫科技风、粒子、扫描、发光、HUD 或旋转效果。
- 动效：以单个 SVG 表达候选人与 AI 之间的双向信息连接、能力节点分析、评估圆环和成长阶梯，页面进入时按背景、品牌、几何、路径、节点、表单顺序渐进呈现；长期动画仅保留 22 秒底图微呼吸、24～29 秒几何慢漂移、10～11 秒低对比节点流动、5 秒评估呼吸和 8.5 秒隐约 A 结构，表单进入只使用 `opacity + translateY`。
- 体验与可访问性：桌面端鼠标视差经平滑插值限制在约 2px / 3.5px / 5.5px，离开或回到中心后复位；移动端隐藏复杂 SVG，暗色模式保持暖色遮罩。`prefers-reduced-motion` 下停止环境循环、SVG 路径节点和视差，路径与能力节点以静态完成状态保留；背景不可聚焦且不进入键盘顺序。输入聚焦、浏览器自动填充、主按钮悬停和按压统一为短时暖棕反馈。
- 验证：应用内浏览器确认 `/login`、`/register`、`/forgot-password` 在 1280×900 与 390×844 下均只有一个背景实例、无横向溢出，移动端 SVG 已关闭；桌面边缘视差实测约 1.97px / 3.45px / 5.41px，回到中心后归零。登录、验证码登录、个人注册、找回密码、减少动态效果与账户密码安全 Playwright 回归 7/7 通过；`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build`、Compose 配置解析和 `git diff --check` 通过，生产构建仅保留既有 CodeEditor 大分块提示。图形验证码在未连接可用后端的开发服务器中显示既有加载失败状态，其认证请求流程由自动化 mock 覆盖。本轮未重建 Docker，未提交、未推送。

### 独立个人注册与安全找回密码流程

- 新增：个人注册从登录卡内状态切换调整为独立 `/register` 页面，按“验证手机号、填写账户信息、注册成功”三步组织；第一步完成手机号、图形验证码和短信验证码准备，第二步填写姓名、用户名、密码及选填邮箱，注册成功后返回登录。页面参考既定留白、横向进度和单列任务层级，继续使用 AInterview 暖色语义 Token、既有表单与按钮体系，未复制外部品牌元素。
- 新增：找回密码调整为独立 `/forgot-password` 页面，按“验证账户、设置新密码、重置完成”三步执行；手机号或邮箱由系统自动识别，图形验证码与验证码发送通过后，只有已启用且联系方式已验证的注册账号才能取得下一步所需的 10 分钟一次性重置票据。登录页的“忘记密码 / 立即注册”改为路由入口，并保留安全 `next` 与可用的联系方式预填。
- 安全：找回密码发送阶段始终返回通用提示，不提供可被批量探测的“账号存在”布尔结果；正确验证码换取的重置票据只保存哈希，绑定用户及当前 `securityVersion`，使用 Redis Lua 原子消费，过期、重复使用或会话安全版本变化均拒绝。密码重置完成后递增安全版本并撤销该用户全部刷新会话；验证码发送、票据签发与密码提交均纳入现有限流、审计和公开端点白名单。
- 体验：注册与找回页面采用真实语义步骤列表、清晰的当前/已完成状态、克制的内容淡入和按钮反馈，`prefers-reduced-motion` 下停用自动过渡；桌面 1280×900 与移动端 390px 已检查，表单保持单一主任务、无横向溢出。设计仅借鉴参考图的结构和节奏，未引入营销区、装饰插图、外部字体或新依赖。
- 测试：根 Maven Reactor 共 360 个测试通过（backend 342、judge worker 18，0 failures / 0 errors / 0 skipped）；前端非真实账号依赖的 Playwright 回归 25/25 通过，覆盖三步注册、联系方式归属验证、一次性票据重置、密码本地校验、三端身份隔离、会话和通知导航；`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build` 通过，生产构建仅保留既有 CodeEditor 大分块提示。完整 Playwright 中依赖真实管理员账号的一组回归因本机未配置 `E2E_IMAGE_CAPTCHA_CODE` 未执行，未将其计为通过。
- 待执行：本轮未新增 Flyway 迁移，尚未重建 Docker，未提交、未推送；当前运行容器不能代表本条独立注册与找回密码界面。

### 登录页文字层级与克制动效优化

- 变更：保留既有单卡片认证结构、暖色语义 Token、圆角和表单流程，在密码登录、验证码登录、注册与找回密码状态下分别补充简短的任务说明；图形验证码增加“安全验证 / 看不清可点击刷新”提示，帮助用户理解当前操作，但不恢复品牌宣传区或额外页面模块。
- 视觉：密码登录与验证码登录使用同一卡片内的滑动下划线表达状态切换；背景只保留低对比度呼吸光晕，卡片首次进入和认证模式切换使用短距离淡入，按钮增加轻量按压反馈。全部自动动效均遵循 `prefers-reduced-motion`，用户选择减少动态效果后会停用背景、卡片和内容动画，并取消页签过渡。
- 清理：删除已不再由当前极简认证页使用的粒子、环形轨道、神经网络、波形、按钮扫光和磁吸卡片样式，避免无效 CSS 与旧视觉方案继续进入生产包；未新增图片、字体或前端依赖。
- 测试：登录/注册、内嵌找回密码、导航与账户安全相关 Playwright 13/13 通过，并新增 390px 无横向溢出和减少动态效果断言；`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build` 通过，生产构建仅保留既有 CodeEditor 大分块提示。应用内浏览器确认 1280px 下卡片仍位于屏幕中央，登录页签指示器、注册和找回密码的同卡片切换均正确。
- 待执行：本轮仅修改工作区前端代码和文档，尚未重建 Docker，未提交、未推送；当前运行容器不能代表本条登录页视觉优化。

### 极简认证卡、自动登录标识识别与图形验证码

- 变更：登录页移除品牌宣传、粒子背景、双栏和其他装饰，仅保留屏幕中央的单一认证卡；卡片最顶部固定为“密码登录 / 验证码登录”两个页签。所有字段说明使用输入框内浅色占位提示，底部仅保留以 `|` 分隔的“忘记密码 | 立即注册”。注册和找回密码均在同一卡片内切换，不再创建弹窗或跳转新页面。
- 变更：密码登录统一接收“用户名 / 手机号 / 邮箱”，服务端按合法邮箱、11 位手机号和历史用户名解析；验证码登录只显示一个“手机号 / 邮箱”输入框，前后端均根据内容自动选择短信或邮件通道，不再让用户手动选择，也不信任兼容请求中的 `channel`。候选人注册继续要求手机号，邮箱为选填，留空时发送验证码和最终注册请求均不提交空字符串字段。
- 安全：删除滑块 challenge/proof 与 `/captcha/verify`，改为服务端生成 PNG 图形验证码。Redis 仅保存答案哈希、用途和客户端 IP 哈希，默认 2 分钟过期；密码登录、登录验证码发送和注册验证码发送分别绑定独立用途，并以 Lua 原子 GET+DEL 一次性消费，答案、用途、IP 任一不匹配即拒绝，Redis 异常时 fail-closed。图形答案为 4–5 位且排除易混淆字符，测试固定码默认关闭，不向前端响应泄露答案。
- 测试：根 Maven Reactor 共 348 个测试通过（backend 330、judge worker 18，0 failures / 0 errors / 0 skipped），其中图形验证码、DTO、登录标识、可选邮箱、自动通道和限流专项 35/35 通过；登录/注册、内嵌找回密码、导航与账户安全相关 Playwright 12/12 通过，`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build` 通过，生产构建仅保留既有 CodeEditor 大分块提示。应用内浏览器确认 1280px 登录卡水平和垂直居中，注册/找回密码切换后仍只有一个主卡片、没有 `dialog`，390px Playwright 验证无横向溢出。
- 待执行：本条图形验证码与极简认证卡代码尚未重新构建到 Docker；当前运行容器仍是上一轮滑块验证码构建，不能代表当前工作区。需在用户明确要求更新 Docker 后，仅重建并以 `--no-deps` 替换 backend/frontend，再验证真实 PNG challenge、Nginx 资源与 `/login`。本轮未新增 Flyway 迁移，未提交、未推送。

### 三端账户路由与候选人面试执行权限隔离

- 安全：候选人、企业、超级管理员三端全部业务路由改为显式声明工作区受众，候选人端不再沿用“只要已登录即可访问”的兜底；管理员或企业账号直接访问候选人页面、候选人访问企业/管理页面时会回到本人工作区。未知、空角色及跨身份域混合角色统一 fail-closed，未知路径不再渲染候选人工作台。
- 安全：应用启动以 `GET /v1/auth/me` 返回的服务端身份为路由授权依据，浏览器 Profile 仅作显示缓存；本地角色缓存被篡改时会被服务端资料覆盖。Access Token 过期时先单飞刷新并同步刷新响应中的用户资料，刷新失败或再次 401 才清理会话；认证服务网络错误或 5xx 时阻断业务页面并提供重试，不使用旧缓存放行。
- 安全：AI 面试的开始、结束、进度保存和逐题作答接口增加 `CANDIDATE` 方法级门禁及服务层本人资源校验，只有面试记录指定的候选人能够推进或代入回答；管理员、企业成员和其他候选人均返回 403。管理员既有面试列表、详情、审核、通过和删除能力保持不变。
- 测试：新增候选人/企业/管理员/未知角色三端访问矩阵、本地角色伪造、非法 `next`、未知路由和混合角色拒绝 E2E，以及管理员/企业/其他候选人写入拒绝与本人开始面试的后端测试；根 Maven Reactor 共 332 个测试通过（0 failures / 0 errors / 0 skipped），前端扩展 Playwright 回归 20/20 通过，`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build`、Compose 配置解析和 `git diff --check` 通过，生产构建仅保留既有 CodeEditor 大分块提示。
- 页面验收：应用内浏览器确认未登录直接访问 `/admin/workspace`、`/company`、`/candidate/jobs` 均保留安全 `next` 并返回登录页，页面无控制台 error；登录后的三端互访由服务端身份 mock 的 E2E 完成验证，未读取、切换或修改现有真实账号会话。未新增 Flyway 迁移，未部署、未提交、未推送。

### 招聘需求、编制预算审批与岗位冻结

- 新增：Flyway V45 建立一岗一单的 `recruitment_requisition` 招聘需求和不可变审批事件账本，记录需求单号、企业、岗位、编制编码/申请人数、成本中心、招聘预算/币种、招聘理由、提交与审核责任人、批准人数、审核意见、冻结原因和状态版本；同一企业的编制编码只能绑定一个招聘需求，防止重复占用。迁移会把历史招聘中/已关闭岗位兼容回填为已批准，把历史草稿回填为待提交，不改变原岗位上下线状态。
- 安全：企业 HR 创建岗位时必须同时填写编制、成本中心、预算和招聘理由，岗位只能保存为草稿；企业原直接发布接口现在返回 403，草稿或已驳回需求通过 `POST /v1/company/recruitment/positions/{id}/submit-approval` 提交后立即锁定。只有 `ADMIN` 可在 `/v1/admin/recruitment/requisitions` 审核，批准人数不能超过申请人数，批准与岗位发布在同一事务中完成；驳回会退回企业修改，关闭仍由企业执行。
- 新增：超级管理员可冻结或解除已批准招聘需求。冻结不破坏岗位、申请和审批历史，但候选人岗位大厅、岗位详情和投递入口都会同时校验“需求已批准且未冻结”，冻结后立即不可浏览或投递；企业端已发布岗位统计同步排除冻结岗位。所有提交、批准、驳回、冻结、解冻均同时写入需求事件账本和平台操作审计。
- 变更：企业岗位创建/编辑页新增“招聘授权”区域，岗位详情和列表以审批状态、编制、成本中心、预算、批准人数、审核/冻结意见及时间线为主层级，并将“发布岗位”改为“提交超级管理员审核”；新增管理端 `/admin/recruitment/requisitions` 审批工作台，支持待办筛选、证据复核、批准并发布、驳回、冻结和解除冻结。界面沿用现有暖色语义 Token、卡片边框、按钮与 Badge 体系。
- 测试：新增 HR 提交锁定、企业直接发布拒绝、管理员批准自动发布、批准人数上限和冻结前置状态测试；根 Maven Reactor 共 324 个测试通过（backend 306、judge worker 18，0 failures / 0 errors / 0 skipped），Flyway 在宿主与 Compose MySQL 8 均验证 39 条迁移并成功升级到 V45，Compose 数据库回填 65 条需求和 65 条事件；`npx.cmd tsc --noEmit`、`npm.cmd run lint` 与 `npm.cmd run build` 通过，生产构建仅保留既有 CodeEditor 大分块提示。
- 运行态验证：仅重建并以 `--no-deps` 替换 backend/frontend，未重启 MySQL、Redis、Gateway 或其他服务，未删除卷；backend 镜像 `sha256:c07ecf191174dade5c964d800f2cc7f5e2a031f383b887f7a087587256661b17` 健康，frontend 镜像 `sha256:f005a4521593701b679d03679d437d861cb07441f7e499217f72cc434ee89678` 正常运行。Nginx 配置、实际审批资源、两个前端路由和管理 API 未认证 401 均已核验；当前浏览器仅有候选人会话，按安全边界未切换或代填管理员/企业凭据，因此本轮未完成登录后的管理端/企业端视觉回归。未提交、未推送。

### 招聘 AI 治理、评测门禁与人工复核闭环

- 新增：Flyway V44 建立招聘 AI 全局/企业策略、合成评测集与用例、评测运行/结果、成本预留和治理事件表，并扩展简历分析、岗位匹配、面试报告及模型调用审计的脱敏、复核、租户、策略版本和成本字段；本地运行库已由 V43 迁移至 V44，已发布迁移未被修改。
- 安全：简历解析、岗位匹配和正式招聘面试逐题评分/报告汇总在发送模型前统一执行 `recruitment-redaction-v1`，递归遮蔽姓名、联系方式、证件号、性别/年龄、婚育家庭、民族/种族、宗教/政治面貌、住址、健康残障和照片等敏感字段，模型输出写入业务表前再次脱敏；新简历分析只持久化脱敏后的提取文本，评测结果只保存响应哈希、断言、分数、漂移和错误，不保存模型原始响应。历史已保存简历正文未做破坏性清理，后续模型调用会先脱敏。
- 新增：招聘 AI 调用统一执行全局与企业双层门禁，企业策略只能收紧全局策略；支持租户禁用、平台紧急停用及原因、精确到 Provider/模型/提示词版本的评测有效期、单请求 Token 上限、全局与租户日/月预算、调用前预留和调用后按实际 Token 结算。30 分钟未结算的崩溃遗留预留会自动释放，企业费率不能低于全局费率规避预算。默认策略启用评测门禁、全部人工复核、敏感字段脱敏，日/月预算分别为 10/200 美元。
- 新增：提供简历分析、岗位匹配和招聘面试评分 3 套合成评测集及 6 个成对样本，按通过率、相对基线漂移和成对公平性分差判定回归；后台异步运行评测，同一运行期间 Provider、模型或提示词版本发生变化会直接判失败，超时运行会自动回收。只有当前 Provider、模型、提示词版本在有效期内通过，才允许对应招聘 AI 调用，控制台的 Go/no-go 状态也使用同一精确门禁。任何失败、过期、预算不足、租户禁用或紧急停用均以治理事件留痕并拒绝调用。
- 变更：岗位匹配新增 `APPROVE / OVERRIDE / DISMISS` 人工复核，覆盖或忽略必须填写说明；需要复核而仍为 `PENDING` 时，不能安排 AI/线下面试，也不能直接流转为录用或拒绝。正式招聘面试报告保存复核人、时间、动作和说明，企业发布动作明确为“复核并发布”；AI 仍只提供辅助建议，不自动淘汰候选人。
- 新增：管理端 `/admin/ai-governance` 汇总招聘 AI 的上线就绪状态、三个评测门禁、全局策略、预算账本、企业禁用和治理事件，支持一键紧急停用、策略更新、企业覆盖及评测运行；界面沿用现有暖色语义 Token，以证据状态和人工责任为主层级，未新增装饰性 AI 渐变视觉体系。管理接口统一位于 `/v1/admin/ai-recruitment-governance` 并记录操作审计。
- 测试：新增敏感字段脱敏、检测即阻断、紧急停用、评测缺失、预算预留和租户费率收紧测试；根 Maven Reactor 共 320 个测试通过（0 failures / 0 errors / 0 skipped），`npx.cmd tsc --noEmit`、`npm.cmd run lint`、`npm.cmd run build`、Compose 配置解析和 `git diff --check` 通过。生产构建仅保留既有 CodeEditor 大分块提示。
- 待执行：V44 只预置合成评测用例，不伪造真实模型通过记录。部署后须由管理员针对当前 Provider、模型和三个提示词版本实际运行并通过全部评测；在此之前招聘简历分析、岗位匹配和正式面试评分会按设计被门禁阻断。Docker Desktop 当前运行状态仍需在最终验收中复核；未执行生产部署、未提交、未推送。
