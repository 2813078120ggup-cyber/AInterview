...
/** Reads result tar files with entry, file-size and path-traversal limits. */
public final class JudgeResultArchiveReader {
    ...
    public static Map<String, byte[]> read(InputStream archiveInput) throws IOException {
        ...
        long totalBytes = 0;
        int entryCount = 0;
        try (BufferedInputStream bufferedInput = new BufferedInputStream(archiveInput);
             TarArchiveInputStream tarInput = new TarArchiveInputStream(bufferedInput)) {
            TarArchiveEntry entry;
            while ((entry = tarInput.getNextEntry()) != null) {
                entryCount++;
                if (entryCount > MAX_ENTRIES) throw new IOException("结果归档条目数量超限");
                if (entry.isDirectory()) continue;
                if (entry.isSymbolicLink() || entry.isLink()) throw new IOException("结果归档禁止包含链接");
                String name = normalizeEntryName(entry.getName());
                long size = entry.getSize();
                if (size < 0 || size > MAX_SINGLE_FILE_BYTES) throw new IOException("结果文件大小非法：" + name);
                totalBytes += size;
                if (totalBytes > MAX_TOTAL_BYTES) throw new IOException("结果归档总大小超限");
